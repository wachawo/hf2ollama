...
<workspace>/llama.cpp/   # 首次转换运行时克隆
```

首次转换运行会把 [`llama.cpp`](https://github.com/ggerganov/llama.cpp)
克隆到工作区内,这样后续运行更快。只有自带预构建 GGUF 文件的仓库
会跳过这一步。如需在多个工作区之间共享同一份克隆,请设置
`HF2OLLAMA_LLAMA_CPP_DIR`(例如 `../llama.cpp`)。
...
```bash
cd llama.cpp   # 或 HF2OLLAMA_LLAMA_CPP_DIR 指向的目录
